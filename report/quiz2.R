...
sources <- list("blogs", "news", "tweets")

# library(microbenchmark)
# microbenchmark(predict_text(questions[[1]], "blogs", n), times = 5L)

answers_blogs <- purrr::map(questions, predict_text, corp = "blogs", return_n = n)
answers_blogs2 <- purrr::map(questions, predict_text, corp = "blogs", return_n = n, keep_stop = TRUE)
